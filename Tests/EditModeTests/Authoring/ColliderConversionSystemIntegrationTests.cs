...
namespace Unity.Physics.Tests.Authoring
{
    class ColliderConversionSystemIntegrationTests : BaseHierarchyConversionTest
    {
        ...
        [Test]
        public void ColliderConversionSystem_StaticHierarchy_OnlyLeafHasCollider_WithUniformScale_ColliderHasExpectedBounds(
            [ValueSource(nameof(GetPrimitiveColliderTypes))] Type colliderType,
            [ValueSource(nameof(GetLocalScalesUniform))] Vector3 localScale)
        {
            List<Type> childComponentTypes = new List<Type> { colliderType, typeof(MeshFilter), typeof(MeshRenderer) };
            CreateHierarchy(
                Array.Empty<Type>(),
                Array.Empty<Type>(),
            ...
            var cubeGameObject = GameObject.CreatePrimitive(PrimitiveType.Cube);
            var cubeMesh = cubeGameObject.GetComponent<MeshFilter>().sharedMesh;
            ...
            Root.isStatic = Parent.isStatic = Child.isStatic = true;

            var expectedBounds = Child.GetComponent<MeshRenderer>().bounds;
            ...
            TestConvertedData<PhysicsCollider>((world, entities, colliders) =>
            {
                ...
                GetRigidBodyTransformationData(ref manager, entity,
                    out var worldTransform, out var scale, out _);

                var colliderBounds = colliders[0].Value.Value.CalculateAabb(worldTransform, scale);
                Assert.That(colliderBounds.Extents, Is.PrettyCloseTo((float3)expectedBounds.size));
                Assert.That(colliderBounds.Center, Is.PrettyCloseTo((float3)expectedBounds.center));
