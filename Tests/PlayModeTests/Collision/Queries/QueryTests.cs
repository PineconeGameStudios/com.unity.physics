...
namespace Unity.Physics.Tests.Collision.Queries
{
    class QueryTests
    {
        ...
        {
#if UNITY_IOS
            Assert.Ignore("This test causes out of memory crashes on iOS, due to the memory restrictions on such devices.");
#elif UNITY_ANDROID
            // Skip on 32-bit Android (armv7) due to out of memory crashes on devices with memory restrictions
            if (IntPtr.Size == 4)
                Assert.Ignore("This test causes out of memory crashes on armv7 builds, due to the memory restrictions on such devices.");
#endif
